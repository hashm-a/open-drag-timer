...
class GPSInterface : public TinyGPSPlus {
public:
    GPSInterface(HardwareSerial& serialPort, long baudRate, uint32_t config, int rxPin, int txPin)
    ...
    void setSatelliteMode(satellite_mode_t mode)
    {
        switch (mode) {
            ...
            case SATELLITE_MODE_QZSS:
                write("$PCAS04,20*2B\r\n");
                break;
            case SATELLITE_MODE_GPSGLONASS:
                write("$PCAS04,5*1C\r\n");
                break;
            case SATELLITE_MODE_GPSGLONASSGALILEO:
                write("$PCAS04,D*6D\r\n");
                break;
    ...
    void setReceiverMode(receiver_mode_t mode)
    {
        switch (mode) {
            case PORTABLE:
                write("$PCAS11,0*1D\r\n");
    ...
    void customWrite(String command)
    {
        write(command.c_str());
    }
